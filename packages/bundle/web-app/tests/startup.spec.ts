...
describe('web command-line provider', () => {
  ...
  it('leaves deployment values to each consumer when flags omit them', async () => {
    const { values, observed } = await bootProvider([])
    expect(values).toEqual({ openBrowser: true, trustedHosts: [], sessionTtlHours: 24, secureCookie: true })
  ...
  it('passes the session TTL and cookie security flags through the provider', async () => {
    const { values, observed } = await bootProvider(['--session-ttl-hours', '8', '--insecure-cookie'])
    expect(values).toEqual({ openBrowser: true, trustedHosts: [], sessionTtlHours: 8, secureCookie: false })
    expect(observed.readerConfig).toEqual({
      host: '127.0.0.1',
      openBrowser: true,
      port: 3080,
