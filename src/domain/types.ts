...
export type RoleName = 'enterprise_owner' | 'product_operator' | 'sales_operator' | 'compliance_operator' | 'fulfillment_operator' | 'service_provider' | 'platform_operator';
...
export interface User extends ScopedRecord { name: string; roleId: ID; status: typeof CANONICAL_STATUSES.User[number]; }
...
export interface Product extends ScopedRecord { name: string; description: string; ownerId: ID; status: ProductLifecycleStatus; currentVersion: number; }
...
export interface Inquiry extends ScopedRecord { customerId: ID; summary: string; status: typeof CANONICAL_STATUSES.Inquiry[number]; }
...
export type DomainAction =
  ...
  | { type: 'quotationFeedback'; actor: Actor; quotationId: ID; feedback: 'viewed' | 'accepted' | 'rejected' | 'negotiating'; idempotencyKey: string }
